...
class PositionalEncoding(layers.Layer):
    ...
    def call(self, x):
        ...
        pos_encoding = self.pos_encoding[tf.newaxis, :, :]
        pos_encoding = tf.tile(pos_encoding, [batch_size, 1, 1])  # Batch 차원에 맞게 확장
...
class SkeletonTransformer(keras.Model):
    ...
    def call(self, x):
        x = tf.transpose(x, perm=[0, 2, 1, 3])  # (batch_size, num_frames, num_joints, 3)
...
def train_with_kfold(model, x_data, y_data, k=5, epochs=10, batch_size=32):
    ...
    model.save("skeleton_transpose_transformer_all.h5")
    model.save('skeleton_transpose_transformer_all.keras')
